...
extension ProductAPIImpl {
    public func fetchProduct(barcode: String, fields: [ProductField]) async throws -> APIProductItem {
        let request = GETProductRequest(barcode, fields: fields)
    ...
    private func convert(_ apiProduct: GETProductResponseElement) -> APIProductItem {
        return APIProductItem(
            ...
            productQuantity: apiProduct.product?.productQuantity,
            productQuantityUnit: apiProduct.product?.productQuantityUnit,
